    ...
    try {
      ...
      const resume = () => {
        try {
          ...
          logoCarousel.slideNext(0); // kleine ‘kick’ zonder overgang
        } catch (e) {
          console.warn('Could not resume autoplay:', e);
        }
      };

      // Swiper events: altijd hervatten
      logoCarousel.on('touchStart', resume);
      logoCarousel.on('touchEnd', resume);
      logoCarousel.on('sliderFirstMove', resume);
      logoCarousel.on('transitionEnd', resume);

      // DOM-events: hervatten (desktop & mobiel)
      ['click', 'pointerdown', 'pointerup', 'pointercancel', 'touchstart', 'touchend', 'touchcancel', 'dragstart', 'contextmenu'].forEach((evt) => {
        container?.addEventListener(evt, resume, { passive: true });
      });

      // Specifieke mobile-guard: blokkeer event-propagatie richting Swiper, maar laat scroll toe
      if (isMobile && container) {
        container.style.touchAction = 'pan-y';
        ['touchstart', 'touchmove', 'touchend', 'pointerdown', 'pointerup', 'click'].forEach((evt) => {
          container.addEventListener(
            evt,
            (e) => {
              e.stopPropagation(); // houdt events weg bij Swiper
              resume(); // en zorg dat autoplay loopt
            },
            { passive: true, capture: true } // capture zodat we vóór Swiper zitten
          );
        });
      }

      // Herstart bij terugkeren naar tab
