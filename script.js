...
try {
  ...
  const resume = () => {
    try {
      // Zorg dat Swiper denkt dat autoplay nooit uit moet
      logoCarousel.params.autoplay.disableOnInteraction = false;
      // Kick autoplay en zet desnoods direct een stap om iOS wakker te schudden
      logoCarousel.autoplay.start();
      if (!logoCarousel.autoplay.running) {
        logoCarousel.autoplay.start();
      }
      // kleine "kick" om beweging te forceren zonder merkbare overgang
  ...
  // Swiper events: als er toch iets binnenkomt, direct hervatten
  ...
  // DOM-events op de container (dek alle mobiele interacties af)
  const events = ['click', 'pointerdown', 'pointerup', 'pointercancel', 'touchstart', 'touchend', 'touchcancel', 'dragstart', 'contextmenu'];
  events.forEach((evt) => container?.addEventListener(evt, resume, { passive: true }));
  ...
  // Force start autoplay na korte delay
  setTimeout(() => {
    resume();
